      ...
      <div className='p-3 max-w-lg mx-auto mb-7 '>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
          ...
          <p className='text-sm self-center'>
            {fileUploadError ? (
              <span className='text-red-700'>
                Greška prilikom postavljanja slike (slika mora biti manja od 2 MB)
              </span>
            ) : filePerc > 0 && filePerc < 100 ? (
              <span className='text-slate-700'>{`Učitavanje ${filePerc}%`}</span>
            ) : filePerc === 100 ? (
              <span className='text-green-700'>Slika uspješno postavljena!</span>
            ) : (
              ''
            )}
          </p>
          ...
          <div class="relative">
            ...
            <input 
              ...
              focus:border-blue-600 peer"  placeholder="Broj telefona " />
          </div>
          <button
            disabled={loading}
          ...
          >
            {loading ? 'Učitavanje...' : 'Ažuriraj profil'}
          </button>
        ...
        <div className='flex justify-between mt-5'>
          <div
            ...
            <span className="absolute flex items-center justify-center w-full h-full text-sky-500 dark:text-white transition-all duration-300 transform group-hover:translate-x-full ease">Obriši račun</span>
            <span className="relative invisible">Obriši račun</span>
          </div>
          <div
            ...
            <span className="absolute flex items-center justify-center w-full h-full text-sky-500 dark:text-white transition-all duration-300 transform group-hover:translate-x-full ease">Odjava</span>
            <span className="relative invisible">Odjava</span>
          </div>
        </div>
        ...
        <p className='text-red-700 mt-5'>
          {showListingsError ? 'Greška prilikom prikaza oglasa' : ''}
        </p>
      </div>
              ...
              <div class="p-5">
                ...
                <button href="#" class="inline-flex items-center px-5 py-2.5 text-sm font-medium text-center text-white bg-sky-700 rounded-lg hover:bg-sky-800 focus:ring-4 focus:outline-none focus:ring-sky-300 dark:bg-amber-600 dark:hover:bg-amber-700 dark:focus:ring-amber-800">
                  <Link to={`/update-listing/${listing._id}`}>
                    <button className='uppercase hover:opacity-50'>Uredi</button>
                  </Link>
                </button>
              </div>
            </div>
